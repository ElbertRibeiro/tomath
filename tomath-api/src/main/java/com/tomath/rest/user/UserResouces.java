...
public class UserResouces {
    ...
    public UserResouces(UserService userService) {
        this.userService = userService;
    }

    @GetMapping
    public ResponseEntity<List<UserDTO>> loadListUsers(){
        return new ResponseEntity<>(ObjectMapperUtils.mapAll(userService.getUsersList(), UserDTO.class), HttpStatus.OK);
    }

    @PostMapping("/create")
    public ResponseEntity<String> insertUsers(@RequestBody UserDTO user){
    ...
    public ResponseEntity<UserDTO> loginUser(@RequestBody UserDTO login){
        return new ResponseEntity<>(ObjectMapperUtils.map(
                userService.loadUser(
                        ObjectMapperUtils.map(login, User.class).getNameUser()), UserDTO.class),
                HttpStatus.OK);
    }
}
